...
class Netex:
    ...
    def line_information(self, line: ET.Element, resource:ET.Element, route_data={}, enum_list:typing.Optional[list[ET.Element]]=None) -> dict:
    ...
    def craftRoutes(self, service:ET.Element, resource:typing.Optional[ET.Element]=None, timetable:typing.Optional[ET.Element]=None, enum_list:typing.Optional[list[ET.Element]]=None):
    ...
    def craftJourneys(self, service:ET.Element, timetable:ET.Element, resource:typing.Optional[ET.Element]=None, enum_list:typing.Optional[list[ET.Element]]=None, epiap_list:typing.Optional[list[Epiap]]=None, loom=True, time_table = True):
        ...
        for journey in journeys:
            ...
            if pattern_ref is not None and patterns is not None:
                ...
                if journey_data['pattern'] not in processed_patterns:
                    pattern = patterns.find(f'./n:ServiceJourneyPattern[@id="{journey_data['pattern']}"]', self.ns)
                    if pattern is not None:
                        ...
                        time_tracker = datetime.strptime("00:00:00", "%H:%M:%S")
                        ...
                        for index, point in enumerate(points):
                            ...
                            if point.tag == f'{{{self.ns["n"]}}}TimingPointInJourneyPattern':
                                ...
                                if timingpoint_ref is not None and timing_points is not None:                                 # BUG
                            ...
                            if quay and time_tracker:
                                ...
                                if index == 0:
                                    pattern_timestamps.append(quay_properties | {
                                        'arrival':None,
                                        'departure':departure
                                    })
            ...
            if time_table and journey_data['line_id']:
                ...
                if len(pattern_timestamps) > 0:
                    pattern_timestamps_df = pd.DataFrame.from_records(pattern_timestamps)
                    pattern_timestamps_df.insert(0, 'pattern', '')
                    pattern_timestamps_df['pattern'] = journey_data['pattern']
                    pattern_timestamps_df.to_sql('pattern_timestamps', self.con, if_exists='append', index=False)
                cur = self.con.cursor()
